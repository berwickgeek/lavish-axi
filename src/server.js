...
// When Lavish is served behind a path prefix (e.g. the launcher reverse-proxies
// /lavish/* to this loopback server), every URL we EMIT must carry the prefix so
// the browser requests /lavish/... . Server routes stay at root; the proxy strips
// the prefix before forwarding. Empty by default → identical standalone behaviour.
const basePath = normalizeBasePath(process.env.LAVISH_AXI_BASE_PATH);
...
export async function serve({
  ...
  app.post("/api/sessions", async (req, res, next) => {
    try {
      ...
      const sessionUrl = `http://${hostForUrl(linkHostName)}:${publicPort}${basePath}/session/${key}`;
  ...
  app.get("/artifact/:key", (req, res) => {
    res.redirect(`${basePath}/artifact/${req.params.key}/index.html`);
  });
  ...
  app.get(/^\/artifact\/([^/]+)\/index\.html$/, async (req, res, next) => {
    try {
      ...
      // The injected SDK (queuePrompt et al.) lives in this HTML — never let a
      // stale copy stick in the browser cache, or decision widgets silently
      // run old code on that device.
      res.set("Cache-Control", "no-cache");
      res.type("html").send(injectLavishSdk(html, key, basePath));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/chrome-client.js", async (req, res, next) => {
    try {
      // no-cache: always revalidate against the ETag so chrome edits reach open
      // pages (esp. iOS Safari, which otherwise caches these heuristically).
      res.set("Cache-Control", "no-cache");
...
// Bumped once per server start. Stamped onto the chrome/SDK asset URLs so a
// deploy forces every client (esp. iOS Safari, which clings to already-cached
// copies even with no-cache) to refetch fresh code exactly once.
...
<link rel="stylesheet" href="${basePath}/chrome.css?v=${ASSET_VERSION}">
</head>
...
<script src="${basePath}/chrome-client.js?v=${ASSET_VERSION}"></script>
</body>
</html>`;
}
