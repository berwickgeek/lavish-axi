...
- Run `npx -y lavish-axi note <html-file> "<progress note>"` while you work on delivered feedback to post a short live step (e.g. "Pricing flights") into the user's "Working" indicator. It returns immediately - it does not wait like poll. Post a few over a long turn so the user sees real progress instead of a bare spinner; notes are ephemeral and reset when the next round of feedback arrives
